/****************************************************************************
** Meta object code from reading C++ file 'qtextorientationspinboxwidget.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.15.19)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include <memory>
#include "../../../../sources/qtextorientationspinboxwidget.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'qtextorientationspinboxwidget.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.15.19. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_QTextOrientationSpinBoxWidget_t {
    QByteArrayData data[7];
    char stringdata0[101];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_QTextOrientationSpinBoxWidget_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_QTextOrientationSpinBoxWidget_t qt_meta_stringdata_QTextOrientationSpinBoxWidget = {
    {
QT_MOC_LITERAL(0, 0, 29), // "QTextOrientationSpinBoxWidget"
QT_MOC_LITERAL(1, 30, 15), // "editingFinished"
QT_MOC_LITERAL(2, 46, 0), // ""
QT_MOC_LITERAL(3, 47, 14), // "setOrientation"
QT_MOC_LITERAL(4, 62, 8), // "setValue"
QT_MOC_LITERAL(5, 71, 11), // "setReadOnly"
QT_MOC_LITERAL(6, 83, 17) // "emitChangeSignals"

    },
    "QTextOrientationSpinBoxWidget\0"
    "editingFinished\0\0setOrientation\0"
    "setValue\0setReadOnly\0emitChangeSignals"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_QTextOrientationSpinBoxWidget[] = {

 // content:
       8,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   44,    2, 0x06 /* Public */,
       1,    0,   47,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       3,    1,   48,    2, 0x0a /* Public */,
       4,    1,   51,    2, 0x0a /* Public */,
       5,    1,   54,    2, 0x0a /* Public */,
       6,    0,   57,    2, 0x08 /* Private */,

 // signals: parameters
    QMetaType::Void, QMetaType::Double,    2,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void, QMetaType::Double,    2,
    QMetaType::Void, QMetaType::Double,    2,
    QMetaType::Void, QMetaType::Bool,    2,
    QMetaType::Void,

       0        // eod
};

void QTextOrientationSpinBoxWidget::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto *_t = static_cast<QTextOrientationSpinBoxWidget *>(_o);
        (void)_t;
        switch (_id) {
        case 0: _t->editingFinished((*reinterpret_cast< double(*)>(_a[1]))); break;
        case 1: _t->editingFinished(); break;
        case 2: _t->setOrientation((*reinterpret_cast< const double(*)>(_a[1]))); break;
        case 3: _t->setValue((*reinterpret_cast< const double(*)>(_a[1]))); break;
        case 4: _t->setReadOnly((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 5: _t->emitChangeSignals(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            using _t = void (QTextOrientationSpinBoxWidget::*)(double );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&QTextOrientationSpinBoxWidget::editingFinished)) {
                *result = 0;
                return;
            }
        }
        {
            using _t = void (QTextOrientationSpinBoxWidget::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&QTextOrientationSpinBoxWidget::editingFinished)) {
                *result = 1;
                return;
            }
        }
    }
}

QT_INIT_METAOBJECT const QMetaObject QTextOrientationSpinBoxWidget::staticMetaObject = { {
    QMetaObject::SuperData::link<QWidget::staticMetaObject>(),
    qt_meta_stringdata_QTextOrientationSpinBoxWidget.data,
    qt_meta_data_QTextOrientationSpinBoxWidget,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *QTextOrientationSpinBoxWidget::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *QTextOrientationSpinBoxWidget::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_QTextOrientationSpinBoxWidget.stringdata0))
        return static_cast<void*>(this);
    return QWidget::qt_metacast(_clname);
}

int QTextOrientationSpinBoxWidget::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 6)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 6;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 6)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 6;
    }
    return _id;
}

// SIGNAL 0
void QTextOrientationSpinBoxWidget::editingFinished(double _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void QTextOrientationSpinBoxWidget::editingFinished()
{
    QMetaObject::activate(this, &staticMetaObject, 1, nullptr);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
